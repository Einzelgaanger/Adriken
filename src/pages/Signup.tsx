            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (isCustomDomain) {
                  ...
                  if (error) { toast.error("Google sign-up failed", { description: error.message }); return; }
                  if (data?.url) {
                    const oauthUrl = new URL(data.url);
                    const allowed = ["accounts.google.com", "wzppyqkgluskxiakdtiw.supabase.co"];
                    if (allowed.some(h => oauthUrl.hostname.includes(h))) { window.location.href = data.url; }
                    else { toast.error("Invalid OAuth redirect"); }
                  }
                } else {
