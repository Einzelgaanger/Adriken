            ...
            <form onSubmit={handleSubmit} className="relative z-10 space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <User className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground/50 pointer-events-none" />
                  <Input id="signup-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your full name" className="pl-10 h-12 sm:h-11 rounded-xl border-border/60 bg-background" required />
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="signup-email" className="text-foreground/80 font-medium text-sm">Email</Label>
                <div className="relative">
                  <Mail className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground/50 pointer-events-none" />
                  <Input id="signup-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="pl-10 h-12 sm:h-11 rounded-xl border-border/60 bg-background" required />
                </div>
              </div>
              <div className="space-y-2">
              ...
              {password.length > 0 && (
                <div className="rounded-xl border border-border/60 bg-background/80 p-3.5 space-y-3">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className={`font-semibold ${passwordStrength.strength === "weak" ? "text-red-500" : passwordStrength.strength === "medium" ? "text-amber-500" : "text-emerald-600"}`}>
                      {passwordStrength.strength.toUpperCase()}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className={`h-1.5 w-8 rounded-full transition-colors ${passwordStrength.score >= i ? (passwordStrength.score < 3 ? "bg-rose-400" : passwordStrength.score < 4 ? "bg-amber-400" : "bg-emerald-500") : "bg-slate-200 dark:bg-navy-600"}`} />
                    ))}
                  </div>
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
                    {Object.entries(passwordStrength.checks).map(([key, value]) => (
                      <span key={key} className={`text-[11px] flex items-center gap-1 ${value ? "text-slate-600 dark:text-slate-300" : "text-slate-400 dark:text-navy-500"}`}>
              ...
              <Button variant="hero" className="w-full h-12 rounded-xl text-base font-bold touch-manipulation" type="submit" disabled={loading || !name.trim() || !email.trim() || passwordStrength.score < 3 || !passwordsMatch || !consent}>
