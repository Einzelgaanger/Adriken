...
const CHART_DAYS = 30;
const chartConfig = { views: { label: "Page views", color: "hsl(217 91% 60%)" } };
...
export default function AdminPageViews() {
  ...
  const chartData = useMemo(() => daily.slice(-CHART_DAYS).map(([date, views]) => ({ date, views })), [daily]);
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Total: {total}. Chart: last {CHART_DAYS} days.</p>
      </div>
      <Card className="border-border/80 shadow-soft rounded-xl">
        <CardHeader>
          <CardTitle>Views per day (last {CHART_DAYS} days)</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading && (
          ...
          {!isLoading && !error && chartData.length === 0 && (
            <p className="text-muted-foreground text-sm py-4">No data for the last {CHART_DAYS} days.</p>
          )}
        </CardContent>
      </Card>
      <Card className="border-border/80 shadow-soft rounded-xl">
        <CardHeader>
          <CardTitle>Views per day (full table, last 90 days)</CardTitle>
        </CardHeader>
        <CardContent>
          {!isLoading && !error && (
